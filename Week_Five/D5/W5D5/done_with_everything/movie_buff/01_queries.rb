...
def harrison_ford
  ...
  Movie.select(:id, :title).joins(:actors).where.not(castings: {ord: 1}).where(actors: {name: 'Harrison Ford'})
...
def biggest_cast
  ...
  Movie.select(:id, :title).joins(:actors).group('movies.id').order('COUNT(actors) DESC').limit(3)
end
